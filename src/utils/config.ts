...
import { WentConfig } from '../types/went.config';
 ...
 */
export function readWentConfig(configPathOption?: string): WentConfig {
  ...
  const targetConfigPath = configPathOption 
                           ? path.resolve(userProjectDir, configPathOption) 
                           : path.resolve(userProjectDir, targetConfigFilename);
  ...
  try {
    ...
    const userConfig: WentConfig = userConfigModule.default || userConfigModule;
